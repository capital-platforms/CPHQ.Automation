...
test('Login Functionacreateacclity', async ({ page }) => {
 ...
 // await page.getByLabel('ABF - ABSOLUTE FINANCIAL SOLUTIONS').click();
  ...
  await page.getByLabel('Trust Account').click();
...
const MobNo = '234234423'
  ...
  const desiredText = h1Text.substring(hyphenIndex + 1).trim();
   ...
   // await expect(page.locator(h1Text)).toBeVisible()
//'await expect(page.getByText(h1Text)).toBeVisible();

// Playwright automatically waits for this button to be actionable.

//await page.getByRole('link', { name: 'View All Accounts' }).click();
//await page.driver.findElement(By.xpath("//tbody/tr[1]"))
 // await page.getByRole('row', { name: 'test test ABF00204 Single' }).getByRole('button').nth(1).click();
...
//const hyphenIndex = h1Text.indexOf('-');
//const desiredText = h1Text.substring(hyphenIndex + 1).trim();
 //const h1Text = await page.locator('xpath=//h1[1]').innerText();

  // Get the inner text of the h1 element
  //const h1Text = await h1Locator.innerText();

 // const part1 = tett.substring(12, 21); 
 //if (typeof tett === 'string' || Array.isArray(tett)) {
        // Now you can safely use indexOf()
      //  const hyphenIndex = tett.indexOf('-');
      // const desiredText = tett.substring(hyphenIndex + 1).trim();
    //} 
   //await page.pause(20000);
