...
test('Commission', async ({ page }) => {
      const login = new LoginPage(page);
      await login.goto();
      await login.login('justin.barrett','AsWmQhbGqjgU');

      const menus = new Menus(page);
       await menus.MovReportloginmenu( 'Reports', 'Moventum Reports' , 'Commission');
 await page.pause(2000)
await page.getByRole('textbox', { name: 'Name: (Optional)' }).fill('Commission Report by Automation testing')
